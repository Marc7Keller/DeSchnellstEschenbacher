...
else
{
	foreach ($_POST['kategorie'] as &$value) {
		   ...
		   $sql= "SELECT * FROM `laptimes` inner join `participants` on fs_participant = participant_id inner join person on fs_person = person_id where fs_event = ".$_SESSION['event']." and fs_category = ".$value." and first_lap != 0 and second_lap != 0 order by first_lap asc LIMIT 4";
			...
			while($row = mysqli_fetch_array($res)){
				...
				$sql_rang = "SELECT * FROM `laptimes` inner join `participants` on fs_participant = participant_id inner join person on fs_person = person_id where fs_event = ".$_SESSION['event']." and fs_category = ".$row['fs_category']." and first_lap != 0 and second_lap != 0 order by second_lap asc;";
